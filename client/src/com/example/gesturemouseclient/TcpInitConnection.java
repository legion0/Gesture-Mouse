...
public class TcpInitConnection extends AsyncTask<Void,Void,Integer> {
	 ...
	 * @param mainActivity
	 */
	public TcpInitConnection(DeviceItem device, String deviceName,MainActivity mainActivity) {
	...
	protected Integer doInBackground(Void... params) {
		...
		TcpClient client = new TcpClient(response,tcp_outgoing_port,deviceName,address);
		client.setTimeout(5);
		int serverUdp;
		try {
			...
			Logger.printLog("TcpInitialConnection", "Failed to find TCP connection.");
			return null;
		}
	}
	...
	static class MyResponseReader implements ResponseReader {
		...
		private final RawValue udp_port = ValueFactory.createRawValue("udp_port".getBytes());

		@Override
		public void read(Value extra_info) {
			udpPort = extra_info.asMapValue().get(udp_port).asRawValue().getString();
		}
	};
